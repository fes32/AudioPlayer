...
using Core;
using Core.Datas;
using Music_Studio.Controls;
using MSUtils = Core.MSUtils;

namespace Music_Studio
{
    public partial class MainForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        public readonly string Filter = MSUtils.DialogAudioFilter;

        private readonly Dictionary<TreeListNode, Data> _dict = new Dictionary<TreeListNode, Data>();
        ...
        public MainForm()
        {
            ...
            if (Bass.BASS_Init(-1, 44100, BASSInit.BASS_DEVICE_LATENCY, Handle, null))
            {
            ...
            _updateTimer.Tick += TimerUpdateTick;
            _sync = new SYNCPROC(EndPosition);
            player1.ButtonClicked += Player1ButtonClicked;
            trackVolume.Value = (int) (Bass.BASS_GetVolume()*100);
        }
      
        #region Bass net

        const bool Hasb = false;
        ...
        private void Play()
        {
            ...
            if (_fileName != String.Empty)
            {
                ...
                if (_stream != 0)
                {
                    ...
                    // trackBarControl1.Properties.Maximum = (int)Bass.BASS_ChannelBytes2Seconds(_stream, len); // the total time length
                    // used in RMS
                    ...
                    if (WF2 != null && WF2.IsRendered)
                    {
                        ...
                        Console.WriteLine("CueIn at {0}sec.; CueOut at {1}sec.", WF2.Frame2Seconds(cueinFrame),
                                          WF2.Frame2Seconds(cueoutFrame));
                        ...
                        if (cueout >= 0)
                        {
                            ...
                            _syncer = Bass.BASS_ChannelSetSync(_stream, BASSSync.BASS_SYNC_POS, cueout, _sync,
                                                               IntPtr.Zero);
                        }
                    }
                }
...
#warning smth
                    ...
                    if (BassTags.BASS_TAG_GetFromFile(_stream, tagInfo))
                    {
                        ...
                        //memoEdit1.Text = tagInfo.album;
                        //memoEdit1.Text += "/n" + tagInfo.artist;
                        //memoEdit1.Text += "/n" + tagInfo.title;
                        //memoEdit1.Text += "/n" + tagInfo.comment;
                        //memoEdit1.Text += "/n" + tagInfo.genre;
                        //memoEdit1.Text += "/n" + tagInfo.year;
                        //memoEdit1.Text += "/n" + tagInfo.track;
                        //pictureBoxTagImage.Image = tagInfo.PictureGetImage(0);
                        //textBoxPicDescr.Text = tagInfo.PictureGetDescription(0);
                        //if (textBoxPicDescr.Text == String.Empty)
                        //    textBoxPicDescr.Text = tagInfo.PictureGetType(0);
                    }
        ...
        #region Local Variables For BassNet...
        private int _stream;
        private string _fileName = String.Empty;
        private int _tickCounter = 0;
        private float _gainDB;
        private DSPPROC _myDSPAddr;
        private readonly SYNCPROC _sync;
        private int _syncer;
        private readonly int _deviceLatencyMS = 0; // device latency in milliseconds
        ...
        private void RMS(int channel, out int peakL, out int peakR)
        {
            ...
            //*//
            //proLeftChanel.Position = peakL;
            //proRightChanel.Position = peakR;
        }
        ...
        private void RMS_2(int channel, out int peakL, out int peakR)
        {
            ...
            var hGC = GCHandle.Alloc(_rmsData, GCHandleType.Pinned);
            try
            {
        ...
        private void GetWaveForm()
        {
            ...
            _zoomed = false;
            // render a wave form
            WF2 = new WaveForm(_fileName, new WAVEFORMPROC(MyWaveFormCallback), this);
        ...
        {
            //if (WF2 != null)
            //    pictureBox1.BackgroundImage = WF2.CreateBitmap(pictureBox1.Width, pictureBox1.Height, _zoomStart, _zoomEnd, true);
            //else
...
#warning Position
            long pos = 0;
            //long pos = WF2.GetBytePositionFromX(e.X, pictureBox1.Width, _zoomStart, _zoomEnd);
        ...
        private void TimerUpdateTick(object sender, System.EventArgs e)
        {
            ...
            if (_tickCounter == 5)
            {
                ...
                if (!Hasb)
                    trackControl.Value = (int)(pos * trackControl.Properties.Maximum / len);
                tbTime.Text = String.Format("{0:#0.00} / {1:#0.00}", Utils.FixTimespan(elapsedtime, "MMSS"), Utils.FixTimespan(totaltime, "MMSS"), Utils.FixTimespan(remainingtime, "MMSS"));
                //                tbTime.Text = String.Format("{0:#0.00} - Total: {1:#0.00} - Remain: {2:#0.00}", Utils.FixTimespan(elapsedtime, "MMSS"), Utils.FixTimespan(totaltime, "MMSS"), Utils.FixTimespan(remainingtime, "MMSS"));
                //Text = String.Format("Bass-CPU: {0:0.00}% (not including Waves & Spectrum!)", Bass.BASS_GetCPU());
            }
        ...
        private void DrawSpectrum()
        {
            switch (specIdx)
            {
                ...
                case 0:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumDot(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.SteelBlue, Color.Pink, Color.Black, 2, 1, false, false, true);
                    break;
                // normal spectrum (full resolution)
                case 1:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrum(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.SteelBlue, Color.Pink, Color.Black, false, true, true);
                    break;
                // line spectrum (width = resolution)
                case 2:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumLine(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Lime, Color.Red, Color.Black, 2, 2, false, false, false);
                    break;
                // line spectrum (full resolution)
                case 3:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumLine(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.SteelBlue, Color.Pink, Color.Black, 16, 4, false, true, true);
                    break;
                // ellipse spectrum (width = resolution)
                case 4:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumEllipse(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Lime, Color.Red, Color.Black, 1, 2, false, false, false);
                    break;
                // ellipse spectrum (full resolution)
                case 5:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumEllipse(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.SteelBlue, Color.Pink, Color.Black, 2, 4, false, true, true);
                    break;
                // dot spectrum (width = resolution)
                case 6:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumDot(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Lime, Color.Red, Color.Black, 1, 0, false, false, false);
                    break;
                // dot spectrum (full resolution)
                case 7:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrum(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Lime, Color.Red, Color.Black, false, false, false);
                    break;
                // peak spectrum (width = resolution)
                case 8:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumLinePeak(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.SeaGreen, Color.LightGreen, Color.Orange, Color.Black, 2, 1, 2, 10, false, false, false);
                    break;
                // peak spectrum (full resolution)
                case 9:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumLinePeak(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.GreenYellow, Color.RoyalBlue, Color.DarkOrange, Color.Black, 23, 5, 3, 5, false, true, true);
                    break;
                // wave spectrum (width = resolution)
                case 10:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumWave(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Yellow, Color.Orange, Color.Black, 1, false, false, false);
                    break;
                // dancing beans spectrum (width = resolution)
                case 11:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumBean(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Chocolate, Color.DarkGoldenrod, Color.Black, 4, false, false, true);
                    break;
                // dancing text spectrum (width = resolution)
                case 12:
                    pictureBoxSpectrum.Image = _vis.CreateSpectrumText(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.White, Color.Tomato, Color.Black, "BASS .NET IS GREAT PIECE! UN4SEEN ROCKS...", false, false, true);
                    break;
                // frequency detection
                case 13:
                ...
                case 14:
                    ...
                    _vis.CreateSpectrum3DVoicePrint(_stream, g, new Rectangle(0, 0, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height), Color.Black, Color.White, voicePrintIdx, false, false);
                    g.Dispose();
                ...
                // WaveForm
                case 15:
                    pictureBoxSpectrum.Image = _vis.CreateWaveForm(_stream, pictureBoxSpectrum.Width, pictureBoxSpectrum.Height, Color.Green, Color.Red, Color.Gray, Color.Black, 1, true, false, true);
                    break;
            }
        }
        #endregion
        ...
        private void trackBarControl1_EditValueChanged(object sender, EventArgs e)
        {
            if (WF2 == null)
                return;
            long len = Bass.BASS_ChannelGetLength(_stream); // length in bytes
            Bass.BASS_ChannelSetPosition(_stream, trackControl.Value * len / trackControl.Properties.Maximum);
        ...
        private void trackVolume_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            //Text = trackVolume.Value.ToString();
            Bass.BASS_SetVolume(trackVolume.Value/100f);
        }

        private void simpleButton2_Click(object sender, EventArgs e)
        {
            Text = Bass.BASS_GetVolume().ToString();
        }
    }
}